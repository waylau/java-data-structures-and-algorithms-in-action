...
public class Ant implements Cloneable {
	...
	public void selectNextCity(float[][] pheromone) {
		...
		for (Integer i : allowedCities) {
			sum += Math.pow(
					pheromone[currentCity][i.intValue()],
					alpha)

					* Math.pow(1.0 / distance[currentCity][i
							.intValue()], beta);
		}
		...
		for (int i = 0; i < cityNum; i++) {
			...
			for (Integer j : allowedCities) {
				if (i == j.intValue()) {
					p[i] = (float) (Math.pow(
							pheromone[currentCity][i],
							alpha)

							* Math.pow(1.0
									/ distance[currentCity][i],
									beta))
							/ sum;
	...
	private int calculateTourLength() {
		...
		for (int i = 0; i < cityNum; i++) {
			len += distance[this.tabu.get(i)
					.intValue()][this.tabu.get(i + 1)
							.intValue()];
		}

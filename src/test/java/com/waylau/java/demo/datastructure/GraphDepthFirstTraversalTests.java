...
/**
 * Graph Depth First Traversal Tests
 * 
...
class GraphDepthFirstTraversalTests {
	...
	void testDepthFirstTraversal() {
		...
		final Graph.Vertex<Integer>[] result = GraphDepthFirstTraversal
				.depthFirstTraversal(undirected.graph,
						undirected.v2);
		...
		assertTrue(result[4].getValue() == 5);
		assertTrue(result[5].getValue() == 6);
		assertTrue(result[6].getValue() == 7);
		assertTrue(result[7].getValue() == 8);
	}
}
